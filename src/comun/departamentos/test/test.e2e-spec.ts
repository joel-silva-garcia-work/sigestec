...
const generateInvalidDTO = (
  ...
  describe('API Tests', () => {
    ...
    describe('Basic CRUD', () => {
      ...
      test('should get an active departamento by ID', async () => {
      ...
      test('should set an active/inactive departamento by ID', async () => {
      ...
      test('should edit a departamento', async () => {
        ...
        const updDto: any = { ...dto, id: departamentoId }
    ...
    describe('Failed Cases', () => {
      describe('Failed Created Cases', () => {
       ...
       describe('Edge Cases', () => {
        ...
        const longNameDto = generateDepartamento(rules)
    ...
    test('should handle very long names in edit', async () => {
      ...
      const longDto: any = { ...longNameDto, id: departamentoId }
    ...
    test('should handle very long descriptions', async () => {
      ...
      const longDescDto = generateDepartamento(rules)
    ...
    test('should handle very long descriptions in edit', async () => {
      ...
      const longDescDto = generateDepartamento(rules)
      longDescDto.description = 'a'.repeat(5000) // Descripción muy larga
      const longDto: any = { ...longDescDto, id: departamentoId }
      const result = await updateData(updateUrl, longDto)
      // Debería fallar con validación de longitud
      expect(result).toBeInstanceOf(HttpError)
      expect(result.status).toBe(400)
    ...
    test('should validate business rules in service layer', async () => {
        ...
        // Crear departamento con nombre duplicado
        const dupeDto = generateDepartamento(rules);
    ...
    test('should fail when editing second departamento with first departamento name', async () => {
        const create_url = url + '/adicionar';
        const update_url = url + '/actualizar';
        ...
        const firstDto = generateDepartamento(rules1);
        const firstResult = await createData(create_url, firstDto);
        const firstId = firstResult.data.id;
        // Crear segundo departamento con nombre diferente
        const secondDto = generateDepartamento(rules1);
        const secondResult = await createData(create_url, secondDto);
        const secondId = secondResult.data.id;
        // Intentar editar el segundo departamento con el nombre del primero
        ...
        const updateDto = {...generateDepartamento(updateRules), 
            id: secondId,
            name: firstDto.name};
        ...
        // console.log(`Test ${expect.getState().currentTestName}: Attempted to update departamento ${secondId} with name '${firstDto.name}' (from departamento ${firstId}) - Operation failed as expected with code ${updateResult.returnCode}`);    
    })
